...
class Planet():
    def __init__(self, x, y, radius, mass, id):
        ...
        self.mass = mass
        self.id = id
        self.rect = pygame.Rect(self.x - self.radius/1.5, self.y - self.radius/1.5, self.radius * 1.5, self.radius * 1.5)
        self.last_pos = (x, y)
        self.velocity = (0, 0)
        self.doneCreating = False
        self.color = random.choice([(236, 37, 37), (236, 151, 37),
                                    (247, 219, 41), (41, 247, 72),
                                    (46, 231, 208), (46, 63, 231),
    ...
    def update(self):
        ...
        if self.doneCreating:
            if self.velocity[0] != 0 and self.velocity[1] != 0:
                particles.append(Particle(self.x, self.y, [np.subtract(self.color, (35, 35, 35))], 1, int(-self.velocity[0]), 1, int(-self.velocity[1]), 1, int(self.radius/1.5), 0.4, 0))
    ...
    def draw(self):
        pygame.draw.circle(display, self.color, (int(self.x), int(self.y)), int(self.radius))
...
while True:
    ...
    for planet in planets:
        if len(planets) >= 50:
            planets.remove(planet)
        if 50000 < planet.x or planet.x < -50000:
            planets.remove(planet)
        if 50000 < planet.y or planet.y < -50000:
            planets.remove(planet)
        if 1000 < planet.velocity[0] or planet.velocity[0] < -1000:
            planets.remove(planet)
        if 1000 < planet.velocity[1] or planet.velocity[1] < -1000:
            planets.remove(planet)
